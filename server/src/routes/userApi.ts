...
router.get("/grocery", async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const items = await repository.find({ where: { userId: req.userId! }, order: { id: "ASC" } });
        return res.json({
            success: true,
            data: items
        });
...
router.post("/grocery", async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const existing = await repository
            ...
            .where("grocery.user_id = :userId", { userId: req.userId! })
            .andWhere("LOWER(grocery.name) = LOWER(:name)", { name: normalizedName })
            .getOne();
        ...
        try {
            ...
            if (error?.code === "ER_DUP_ENTRY") {
                const duplicatedItem = await repository
                    .createQueryBuilder("grocery")
                    .where("grocery.user_id = :userId", { userId: req.userId! })
                    .andWhere("LOWER(grocery.name) = LOWER(:name)", { name: normalizedName })
                    .getOne();
                ...
                if (duplicatedItem && !Boolean(custom)) {
                    ...
                    const updated = await repository.save(duplicatedItem);
                    return res.json({
                        success: true,
                        data: updated
                    });
...
router.patch("/grocery/:id", async (req: AuthenticatedRequest, res: Response) => {
    try {
        const repository = AppDataSource.getRepository(GroceryItem);
        const item = await repository.findOne({ where: { id: Number(req.params.id), userId: req.userId! } });
        if (!item) {
