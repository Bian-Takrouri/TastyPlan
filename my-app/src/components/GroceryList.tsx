...
export function GroceryList({ mealPlannerState }: Props) {
    ...
    const generatedItems = useMemo(() => {
        ...
        for (const item of Object.values(mealPlannerState)) {
            ...
            for (let i = 1; i <= 20; i++) {
                const ingredient = item.meal[
                    `strIngredient${i}` as keyof typeof item.meal
                ];
                ...
                if (ingredient && String(ingredient).trim()) {
                    ...
                    ingredientCounts.set(
                        key,
                        (ingredientCounts.get(key) ?? 0) + 1
                    );
                }
            }
        }

        return Array.from(ingredientCounts.entries()).map(
            ([name, quantity]) => ({
                name,
                quantity
            })
    ...
    async function toggleCheckbox(item: GroceryItem) {
        try {
            const updated = await updateGroceryItem(
                item.id,
                !item.completed
            );
                ...
                <div className="emptyGrocery">
                    {items.length === 0 ? (
                        <>
                            ...
                            <p>
                                Add meals to your planner to generate
                                a shopping list.
                            </p>
                        </>
                    ) : filter === "completed" ? (
