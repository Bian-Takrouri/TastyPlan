...
function App() {
  ...
  useEffect(() => {
    const loadMealPlan = async () => {
      try {
        const items = await new MealAPI().getMealPlan();
        ...
        for (const item of items) {
          ...
          recipe.ingredients?.slice(0, 20).forEach((ingredient: any, index: number) => {
  ...
  return (
    <div className={`hallApp ${theme}`}>
      ...
      <Routes>
        ...
        <Route
          path="/grocery"
          element={<Grocery mealPlannerState={mealPlannerState} />}
        />

        <Route
          path="/favorites"
          element={<Favorites dispatch={dispatch} />}
        />
